		...
		size_t count, loff_t *offset)
{
	...
	//pr_info("%s : read request for %zu bytes.\n", __func__, count);
        ...
        wake_lock_timeout(&pn547_dev->wl, 2 * HZ);
				...
				struct pn547_i2c_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->irq_gpio)) {
		...
		dev_err(dev, "irq_gpio not provided\n");
		goto err_irq;
	}
	
	if (gpio_is_valid(pdata->fwdl_en_gpio)) {
		...
		dev_err(dev, "fwdl_en_gpio not provided\n");
		goto err_irq;
	}
	...
	if (gpio_is_valid(pdata->ven_gpio)) {
		...
		dev_err(dev, "ven_gpio not provided\n");
		goto err_ven;
	}
